...
# Load TCGA transcriptomic data
if (file.exists("tcga_brca.rds")){
  exp_data <- readRDS("tcga_brca.rds")
} else{
  ...
  # Save to RDS
  saveRDS(exp_data, file = "tcga_brca.rds")
}
```
...
# Visualize gene expression spreads for all genes
...
filter_corr_results <- function(df, corr_threshold){
  df[!is.na(df$correlation) & abs(df$correlation) >= corr_threshold, ]
}
